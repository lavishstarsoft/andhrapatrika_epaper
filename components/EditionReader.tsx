...
export default function EditionReader({ initialEdition, alias, pageFlipSoundEnabled = true }: EditionReaderProps) {
  ...
  const handleTouchStart = (e: React.TouchEvent, type: 'move' | 'resize', handle?: string) => {
    ...
    const containerElement = e.currentTarget.closest('.relative') as HTMLDivElement;
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const touch = e.touches[0];
    const { type, handle, startX, startY, startCrop, containerElement } = touchRef.current;
    
    const activeContainer = containerElement || mobileContainerRef.current;
    if (!activeContainer) return;

    const bounds = getImageBounds(activeContainer);
    if (!bounds) return;
  ...
  const handlePointerDown = (e: React.PointerEvent, type: 'move' | 'resize', handle?: string) => {
    ...
    const handlePointerMove = (moveEvent: PointerEvent) => {
      ...
      const bounds = getImageBounds(containerElement);
      if (!bounds) return;
  ...
  return (
    <div className="flex flex-col">
          ...
          <div className="bg-white rounded-sm shadow-xl w-full max-w-2xl flex flex-col overflow-hidden">
            ...
            <div className="p-4 flex flex-col gap-4 overflow-y-auto max-h-[85vh] bg-gray-50">
              ...
              <div className="flex justify-center p-2">
                <div
                  className="bg-white border-[3px] border-black shadow-xl inline-flex flex-col max-w-full"
                >
                  {/* Card Header - Banner Style with Text */}
                  <div className="bg-white flex flex-col border-b border-gray-100 overflow-hidden">
                    <div className="pt-0 pb-0 flex items-center justify-center bg-white -mt-2">
                      <img
                        src="/ys-logo.jpeg"
                        alt="Andhrapatrika"
                    ...
                    <div className="-mt-3 pb-0.5 px-3 text-center text-[11px] font-bold uppercase tracking-tight text-[#2D2D2D]">
      ...
      {isQuickShareOpen && (
        <div
          className="fixed inset-0 z-[210] bg-black/50 backdrop-blur-sm flex items-end md:items-center justify-center p-3 md:p-6"
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 bg-black/50 backdrop-blur-md text-white border-b border-white/10 shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {isCropOpen ? <X size={14} /> : <Crop size={14} />}
                  <span>{isCropOpen ? 'Close Crop' : 'Crop'}</span>
                </button>
                <button
            ...
            <div className="flex-1 relative overflow-hidden bg-black">
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    {/* Crop Overlay inside Desktop Zoom */}
                    {isCropOpen && (
                      <div className="absolute inset-0 z-10 pointer-events-none">
                        <div
              ...
              >
                ...
                >
                  ...
                  {/* Crop Overlay inside Mobile Zoom */}
                  {isCropOpen && (
                    <div className="absolute inset-0 z-10">
                      <div className="absolute inset-0 pointer-events-none">
                        <div
